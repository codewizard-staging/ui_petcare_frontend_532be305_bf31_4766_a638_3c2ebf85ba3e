import PetServiceJsonConfig from "config/petservice_product_information_form_fullform_config.json";
...
const MapItems = [
    { navpropname: "", uicomponent: "petservice", expand: "", exclude: [], func: Support.AddOrUpdatePetService }
]

const FetchPetServiceInfo = async () => {
    let item = {};
    return new Promise(async (resolve) => {
        const keyItems = Object.keys(PetServiceJsonConfig);
        keyItems.forEach(elm => {
            let items = [];
            for (let prop of PetServiceJsonConfig[elm]) {
...
const FetchPetServiceDetails = async (petserviceId, enums) => {
    ...
    return new Promise(async (resolve) => {
        ...
        if (petserviceId) {
            ...
            rslt = await Api.GetPetServiceSingle(petserviceId, `$expand=${$expand}`);
            
            if (rslt.status) {

                const petservice = rslt.values;
                ...
                for (let i = 0; i < MapItems.length; i++) {
                    ...
                    const sourceObj = Helper.IsNullValue(source) ? petservice : petservice[source];
...
const Extract = async (petserviceId) => {

    return new Promise(async (resolve) => {

        let rtnObj = { row: {}, options: [], backRow: {} };

        await FetchPetServiceInfo().then(async (item) => {
            ...
            await FetchDropdownItems(items).then(async (enums) => {
                ...
                if (!Helper.IsNullValue(petserviceId)) {
                    await FetchPetServiceDetails(petserviceId, enums).then(({ row, backRow }) => {
